...
export default function ChatInterface() {
  ...
  const assistantBorder = dark ? COLORS.BORDER.DEFAULT_DARK : COLORS.BORDER.DEFAULT_LIGHT;
  const inputBg = dark ? COLORS.BACKGROUND.SECONDARY_DARK : COLORS.BACKGROUND.SECONDARY_LIGHT;
                ...
                sx={{
                  ...
                  background:
                    msg.role === "user"
                      ...
                      : undefined,
                  backgroundColor: msg.role === "assistant" ? assistantBg : undefined,
                  color: msg.role === "user" ? COLORS.WHITE : (dark ? COLORS.TEXT.PRIMARY_DARK : COLORS.TEXT.PRIMARY_LIGHT),
                  boxShadow:
                    msg.role === "user"
                  ...
                  border: msg.role === "assistant" ? `1px solid ${assistantBorder}` : "none",
                  alignSelf: msg.role === "user" ? "flex-end" : "flex-start",
                }}
              >
                {msg.role === "assistant" && !isGreeting && (
                  <Box sx={{ display: "flex", alignItems: "center", gap: 0.75, mb: 1 }}>
                    <SmartToy sx={{ fontSize: 18, color: COLORS.PRIMARY_PURPLE }} />
                    <Typography variant="caption" sx={{ fontWeight: 600, color: COLORS.PRIMARY_PURPLE }}>
                      Kart AI
                    </Typography>
                  </Box>
                )}
                {isGreeting && (
                  <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 1.5 }}>
                    ...
                    <Typography variant="subtitle2" sx={{ fontWeight: 700, color: dark ? COLORS.TEXT.PRIMARY_DARK : COLORS.TEXT.PRIMARY_LIGHT }}>
                      {t("kartAi")}
                    </Typography>
                  </Box>
                )}
                ...
                <Box sx={{ width: "100%", mt: 1.5 }}>
                  <Typography
                    ...
                    sx={{
                      ...
                      mb: 1.5,
                      fontWeight: 700,
                      color: dark ? COLORS.TEXT.SECONDARY_DARK : COLORS.TEXT.SECONDARY_LIGHT,
                ...
                <Box sx={{ mt: 1.5, width: "100%" }}>
                  ...
                  <Box sx={{ display: "flex", flexDirection: "row", flexWrap: "wrap", gap: 1.5 }}>
                    {msg.suggestedCategories.map((category) => (
                      ...
                      >
                        <Typography variant="subtitle2" sx={{ fontWeight: 700, mb: 1.25, color: dark ? COLORS.TEXT.PRIMARY_DARK : COLORS.TEXT.PRIMARY_LIGHT }}>
                          {category.name}
                        </Typography>
                          ...
                          {category.subcategories.map((subcat) => (
                            <Chip
                              ...
                              sx={{
                                ...
                                fontWeight: 600,
                                fontSize: "0.75rem",
                                "&:hover": { backgroundColor: COLORS.PURPLE_ALPHA_20 },
                              }}
                            />
                          ))}
                        </Box>
          ...
          <Box sx={{ display: "flex", flexDirection: "column", alignItems: "flex-start", width: "100%" }}>
            <Box
              sx={{
                ...
                alignItems: "center",
                gap: 1.5,
                boxShadow: dark ? "0 2px 12px rgba(0,0,0,0.2)" : "0 2px 12px rgba(94, 24, 233, 0.08)",
              }}
            >
              <CircularProgress size={20} sx={{ color: COLORS.PRIMARY_PURPLE }} />
              <Typography variant="body2" sx={{ color: dark ? COLORS.TEXT.SECONDARY_DARK : COLORS.TEXT.SECONDARY_LIGHT, fontWeight: 500 }}>
                {t("ai_thinking")}
              </Typography>
          ...
          <TextField
            ...
            sx={{
              "& .MuiOutlinedInput-root": {
                ...
                "&.Mui-focused fieldset": { borderWidth: 2, borderColor: COLORS.PRIMARY_PURPLE },
              },
            }}
          />
          <IconButton
            ...
            sx={{
              bgcolor: isLoading || !searchValue.trim() ? undefined : COLORS.PRIMARY_PURPLE,
              color: isLoading || !searchValue.trim() ? (dark ? COLORS.TEXT.SECONDARY_DARK : COLORS.TEXT.SECONDARY_LIGHT) : COLORS.WHITE,
              "&:hover": {
                bgcolor: isLoading || !searchValue.trim() ? undefined : COLORS.PURPLE_HOVER,
              },
              "&.Mui-disabled": { bgcolor: "transparent" },
            }}
          >
            {isLoading ? <CircularProgress size={24} color="inherit" /> : <Send sx={{ fontSize: 22 }} />}
          </IconButton>
        </Box>
      </Box>

...
const ReelContainer: React.FC<ReelContainerProps> = ({
  ...
  const isDrawerOpen = useAppSelector(
    (state: any) => state.profileDrawer.isOpen,
  );
  ...
  const handleProfileClick = () => {
    dispatch(
      openDrawer({
        userId: reel.user.id,
        role:
          ((reel.upload_user_type || reel.user.role) as AppUserType) ||
          AppUserType.SERVICE_PROVIDER,
  ...
  return (
    <Box
      sx={{
        height: {
          xs: "calc(100vh - 70px - 60px)",
          sm: "calc(100vh - 64px - 60px)",
          md: "calc(100vh - 72px)",
        },
        width: "100%",
        display: "flex",

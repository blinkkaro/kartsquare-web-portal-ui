...
const ServicePricingOptions: React.FC<ServicePricingOptionsProps> = ({
    ...
    const optionCardStyle = (active: boolean) => ({
        ...
        bgcolor: active ? (isDark ? COLORS.PURPLE_ALPHA_10 : COLORS.PURPLE_ALPHA_04) : "transparent",
        cursor: "pointer",
        ...
        <Box sx={{ mb: 3 }}>
            ...
            <Box
                sx={{
                    ...
                    p: 1.5,
                    borderRadius: 1.5,
                    bgcolor: isDark ? alpha(COLORS.PRIMARY_PURPLE, 0.08) : alpha(COLORS.PRIMARY_PURPLE, 0.06),
                    border: `1px solid ${isDark ? alpha(COLORS.PRIMARY_PURPLE, 0.2) : alpha(COLORS.PRIMARY_PURPLE, 0.15)}`,
                }}
            >
                <InfoOutlinedIcon sx={{ color: COLORS.PRIMARY_PURPLE, fontSize: 20, mt: 0.25 }} />
                <Typography variant="body2" color="text.secondary" sx={{ lineHeight: 1.5 }}>
                    Choose how you want to set your service price(s). You can change this anytime.
                </Typography>
            </Box>
            ...
            <FormControl component="fieldset" sx={{ width: "100%", mb: 2 }}>
                ...
                >
                    <Paper variant="outlined" sx={optionCardStyle(pricingType === "single")} onClick={() => onPricingTypeChange("single")}>
                        ...
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1, mt: -0.5 }}>
                            ...
                            <Typography variant="body2" fontWeight={600}>{english.pricing_single}</Typography>
                        </Box>
                        <Typography variant="caption" color="text.secondary" sx={{ display: "block", mt: 0.5, lineHeight: 1.4 }}>
                            {english.pricing_single_desc}
                        </Typography>
                    </Paper>
                    <Paper variant="outlined" sx={optionCardStyle(pricingType === "catalog")} onClick={() => onPricingTypeChange("catalog")}>
                        ...
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1, mt: -0.5 }}>
                            ...
                            <Typography variant="body2" fontWeight={600}>{english.pricing_catalog}</Typography>
                        </Box>
                        <Typography variant="caption" color="text.secondary" sx={{ display: "block", mt: 0.5, lineHeight: 1.4 }}>
                            {english.pricing_catalog_desc}
                        </Typography>
                    </Paper>
                    <Paper variant="outlined" sx={optionCardStyle(pricingType === "multiple")} onClick={() => onPricingTypeChange("multiple")}>
                        ...
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1, mt: -0.5 }}>
                            ...
                            <Typography variant="body2" fontWeight={600}>{english.pricing_multiple}</Typography>
                        </Box>
                        <Typography variant="caption" color="text.secondary" sx={{ display: "block", mt: 0.5, lineHeight: 1.4 }}>
            ...
            {/* Single price — with helper */}
            {pricingType === "single" && (
                <Paper variant="outlined" sx={{ p: 2, borderRadius: 2, bgcolor: isDark ? COLORS.BACKGROUND.PAPER_DARK : COLORS.BACKGROUND.SECONDARY_LIGHT }}>
                    <Typography variant="body2" sx={{ mb: 0.5, fontWeight: 500 }}>
                        {english.price_inr}
                        <span style={{ color: COLORS.SECONDARY_ORANGE }}>*</span>
                    </Typography>
                    <Typography variant="caption" color="text.secondary" sx={{ mb: 1.5, display: "block" }}>
                        Enter the price in INR. Customers will see this on your service listing.
                    </Typography>
                    <TextField
                        fullWidth
                        size="small"
                        ...
                        onChange={(e) => {
                            ...
                            if (val === "" || (parseFloat(val) <= 10000 && (val.match(/\./g) || []).length <= 1)) onPriceChange(val);
                        }}
            ...
            {pricingType === "catalog" && (
                <Paper
                    ...
                    sx={{
                        p: 2,
                        borderRadius: 2,
                        bgcolor: isDark ? COLORS.BACKGROUND.PAPER_DARK : COLORS.BACKGROUND.SECONDARY_LIGHT,
                        borderColor: errors?.catalog ? "error.main" : undefined,
                    }}
                >
                    <Typography variant="body2" sx={{ mb: 0.5, fontWeight: 500, color: errors?.catalog ? "error.main" : undefined }}>
                        {english.upload_price_catalog}
                        <span style={{ color: COLORS.SECONDARY_ORANGE }}>*</span>
                    </Typography>
                    <Typography variant="caption" color="text.secondary" sx={{ mb: 1.5, display: "block", lineHeight: 1.5 }}>
                        Upload your price list as PDF or image. Customers can view it when they open your service. {english.upload_price_catalog_helper}
                    </Typography>
                    <Box
                        component="label"
                        sx={{
                            ...
                            bgcolor: isDark ? alpha(COLORS.PRIMARY_PURPLE, 0.04) : alpha(COLORS.PRIMARY_PURPLE, 0.03),
                            cursor: "pointer",
                            transition: "border-color 0.2s ease, bgcolor 0.2s ease",
                            "&:hover": { borderColor: COLORS.PRIMARY_PURPLE, bgcolor: isDark ? alpha(COLORS.PRIMARY_PURPLE, 0.08) : alpha(COLORS.PRIMARY_PURPLE, 0.06) },
                        }}
                    >
                        <CloudUploadIcon sx={{ fontSize: 40, color: COLORS.PRIMARY_PURPLE, mb: 1, opacity: 0.9 }} />
                        <Typography variant="body2" fontWeight={600} color={COLORS.PRIMARY_PURPLE} sx={{ mb: 0.5 }}>
                            {english.upload_price_catalog}
                        </Typography>
                        <Typography variant="caption" color="text.secondary">
                            PDF, JPG or PNG · Multiple files allowed
                        </Typography>
                        <input type="file" hidden multiple accept=".pdf,image/jpeg,image/png,image/jpg" onChange={onCatalogFileSelect} />
                    </Box>
                    {errors?.catalog && (
                    ...
                    {(priceCatalogFileNames.length > 0 || existingCatalogUrls.length > 0) && (
                        <Box sx={{ mt: 2 }}>
                            ...
                            <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1 }}>
                                {priceCatalogFileNames.map((name, index) => (
                                    ...
                                    >
                                        ...
                                        <IconButton size="small" onClick={() => onRemoveCatalogFile(index)} color="error" aria-label={english.remove_price_item}>
                                ...
                                {existingCatalogUrls.map((url, index) => (
                                    ...
                                    >
                                        ...
                                        {onRemoveExistingCatalogUrl && (
                                            <IconButton size="small" onClick={() => onRemoveExistingCatalogUrl(index)} color="error" aria-label="Remove">
                                                <DeleteIcon fontSize="small" />
                                            </IconButton>
                                        )}
                                    </Box>
                            ...
                            <Button size="small" onClick={onClearCatalog} sx={{ mt: 1, color: "text.secondary", textTransform: "none" }}>
            ...
            {pricingType === "multiple" && (
                ...
                >
                    ...
                    <Typography variant="caption" color="text.secondary" sx={{ mb: 2, display: "block", lineHeight: 1.5 }}>
                        Enter name, price (INR), and a short description for each item. You can add as many as you need.
                    </Typography>
                    {errors?.multiple && (
                        ...
                        {priceItems.map((item, index) => (
                            ...
                            >
                                <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 1.5 }}>
                                    ...
                                    {priceItems.length > 1 && (
                                        <IconButton size="small" onClick={() => onRemovePriceItem(index)} color="error" aria-label={english.remove_price_item}>
                                            <DeleteIcon fontSize="small" />
                                        </IconButton>
                                    )}
                                </Box>
                                <Box sx={{ mb: 1.5 }}>
                                    ...
                                    <TextField
                                        ...
                                        size="small"
                                        value={item.name}
                                        onChange={(e) => onUpdatePriceItem(index, "name", e.target.value)}
                                ...
                                <Box sx={{ mb: 1.5 }}>
                                    ...
                                    <TextField
                                        ...
                                        onChange={(e) => {
                                            const val = e.target.value.replace(/[^0-9.]/g, "");
                                            if (val === "" || (parseFloat(val) <= 100000 && (val.match(/\./g) || []).length <= 1)) onUpdatePriceItem(index, "price", val);
                                        }}
                                        placeholder={english.enter_price}
                                    />
                                </Box>

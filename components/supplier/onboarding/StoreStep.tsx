...
import { Box, Typography, FormControlLabel, Checkbox, Autocomplete, TextField, FormGroup, Chip, Grid, useTheme, Paper, InputAdornment, MenuItem } from "@mui/material";
...
const StoreStep: React.FC<StoreStepProps> = ({ onNext, onBack }) => {
    ...
    const { data: addresses, isLoading: isLoadingAddresses, refetch: refetchAddresses } = useGetAddress();
    ...
    const handleImageChange = async (files: (File | string)[], field: 'logo_url' | 'banner_url') => {
                ...
                <Grid container spacing={4}>
                    ...
                    <Grid size={{ xs: 12 }}>
                        <Typography variant="subtitle1" fontWeight="600" mb={1} display="flex" alignItems="center" gap={1}>
                            <BusinessOutlinedIcon fontSize="small" color="primary" /> Basic Information
                        </Typography>
                    </Grid>

                    <Grid size={{ xs: 12, md: 6 }}>
                        <Typography variant="body2" fontWeight="500" mb={0.5}>Store Display Name*</Typography>
                        <Input name="display_name" control={control} placeholder="LALA JI KELE WALE" startIcon={<StoreOutlinedIcon />} />
                    </Grid>
                    <Grid size={{ xs: 12, md: 6 }}>
                        <Typography variant="body2" fontWeight="500" mb={0.5}>Store Website URL*</Typography>
                        <Input name="slug" control={control} placeholder="my-store-name" startIcon={<LinkOutlinedIcon />} />
                    </Grid>
                    <Grid size={{ xs: 12 }}>
                    ...
                    <Grid size={{ xs: 12, md: 6 }}>
                        <Typography variant="body2" fontWeight="500" mb={0.5}>Categories Served*</Typography>
                        <Controller
                                    ...
                                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mt: 1.5 }}>
                                        {((value as string[]) || []).map((option) => (
                                            <Chip
                                                key={option}
                                                label={option}
                                                onDelete={() => onChange((value as string[]).filter(v => v !== option))}
                                                variant="filled"
                                                size="medium"
                                                sx={{
                    ...
                    <Grid size={{ xs: 12, md: 6 }}>
                        <Typography variant="body2" fontWeight="500" mb={0.5}>Operating Locations*</Typography>
                        <Controller
                    ...
                    {/* Appearance Section */}
                    <Grid size={{ xs: 12 }}>
                        <Typography variant="subtitle1" fontWeight="600" mb={2} mt={5} display="flex" alignItems="center" gap={1}>
                            <InfoOutlinedIcon fontSize="small" color="primary" /> Aesthetics & Branding
                        </Typography>
                    </Grid>
                    ...
                    <Grid size={{ xs: 12 }}>
                        <Typography variant="subtitle1" fontWeight="600" mb={2} mt={5} display="flex" alignItems="center" gap={1}>
                            <LocationOnOutlinedIcon fontSize="small" color="primary" /> Store Location
                        </Typography>
                    </Grid>
                        ...
                        <Box sx={{ mt: 1 }}>
                            {isLoadingAddresses ? (
                                <Box display="flex" gap={2}>
                                    <CircularProgress size={20} /> <Typography variant="body2">Loading addresses...</Typography>
                                </Box>
                            ) : (
                                <Grid container spacing={2}>
                                    {addresses?.map((addr: any) => (
                                        <Grid size={{ xs: 12, md: 6 }} key={addr.id}>
                                            <Box
                                                onClick={() => setValue("store_address_id", addr.id)}
                                                sx={{
                                                    p: 2,
                                                    border: "1px solid",
                                                    borderColor: watch("store_address_id") === addr.id ? COLORS.PRIMARY_PURPLE : "divider",
                                                    borderRadius: 3,
                                                    cursor: "pointer",
                                                    bgcolor: watch("store_address_id") === addr.id ? `${COLORS.PRIMARY_PURPLE}08` : "transparent",
                                            ...
                                            >
                                                <Box sx={{
                                                    ...
                                                    height: 40,
                                                    borderRadius: 2,
                                                    bgcolor: watch("store_address_id") === addr.id ? COLORS.PRIMARY_PURPLE : "divider",
                                                    display: "flex",
                                                    alignItems: "center",
                                                ...
                                                <Box sx={{ flex: 1 }}>
                                                    <Typography variant="subtitle2" fontWeight="700">{addr.address_name}</Typography>
                                                    <Typography variant="caption" color="text.secondary" noWrap sx={{ display: 'block' }}>
                                                        {addr.address}, {addr.city_town}
                                                    </Typography>
                                                </Box>
                                                {watch("store_address_id") === addr.id && (
                                                    <Chip label="Selected" size="small" color="primary" variant="filled" sx={{ height: 20, fontSize: '0.65rem' }} />
                                                )}
                                            </Box>
                                        </Grid>
                                    ))}
                    ...
                    <Grid size={{ xs: 12 }}>
                        <Typography variant="subtitle1" fontWeight="600" mb={2} mt={5} display="flex" alignItems="center" gap={1}>
                            <PhoneOutlinedIcon fontSize="small" color="primary" /> Contact Information
                        </Typography>
                    </Grid>
                    ...
                    <Grid size={{ xs: 12 }}>
                        <Grid container spacing={2.5}>
                            <Grid size={{ xs: 12, md: 7 }}>
                                <Typography variant="body2" fontWeight="500" mb={1}>Primary Contact Number*</Typography>
                            ...
                            <Grid size={{ xs: 12, md: 5 }}>
                                <Typography variant="body2" fontWeight="500" mb={1}>Contact Email</Typography>
                                <Input name="contact_email" control={control} placeholder="business@mail.com" startIcon={<EmailOutlinedIcon fontSize="small" />} type="email" />
                            </Grid>
                        </Grid>
                    </Grid>

                    <Grid size={{ xs: 12, md: 6 }}>
                        <Typography variant="body2" fontWeight="500" mb={1}>Establishment Year</Typography>
                        <Input name="establishment_year" control={control} placeholder="e.g. 2020" startIcon={<CalendarTodayOutlinedIcon fontSize="small" />} />
                    </Grid>
                </Grid>

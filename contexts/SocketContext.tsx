    ...
    <Box
      sx={{
        ...
        animation: t.visible
          ...
          : "animate-leave 0.3s ease-in forwards",
      }}
    >
      <Box sx={{ flex: 1 }}>
...
export const SocketProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  useEffect(() => {
    ...
    newSocket.on("new_notification", (notification: any) => {
      ...
      toast.custom(
        (t) => <NotificationToast notification={notification} t={t} />,
        {
          duration: 3000,
          position: "top-right",
        },
  ...
  const markAllAsRead = async () => {
    ...
    try {
      await axios.patch(
        `${API_URL}/notifications/mark-all-viewed`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const markAsRead = async (id: string) => {
    ...
    try {
      await axios.patch(
        `${API_URL}/notifications/${id}/viewed`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      ...
      setNotifications((prev) =>
        prev.map((n) =>
          n.notification_id === id ? { ...n, is_viewed: true } : n,
        ),
      );

"use client";

import React, { createContext, useContext, useState, ReactNode } from "react";
import { english } from "./en";
// Define available languages
// export type Locale = "en" | "es" | "hi";

// Define translation keys (example)
export type TranslationKey =
  | "valNameMax"
  | "valDescMax"
  | "valPriceMax"
  | "valPriceMin"
  | "valEmailMax"
  | "valPhoneExact"
  | "valQtyMax"
  | "valOtpMax"
  | "kyc_owner_name_required"
  | "store_setup_business_type_required"
  | "welcome_back"
  | "email_address"
  | "password"
  | "forgot_password"
  | "login"
  | "continue_with_google"
  | "no_account"
  | "sign_up"
  | "brand_tagline"
  | "continue_as"
  | "customer"
  | "service_provider"
  | "supplier"
  | "login_subtitle"
  | "verifyEmailTitle"
  | "verifyEmailSubTitle"
  | "verifyDocumentTitle"
  | "verifyDocumentSubTitle"
  | "home_banner_eyebrow"
  | "home_banner_title"
  | "home_banner_subtitle"
  | "home_banner_search_placeholder"
  | "home_banner_search_button"
  | "home_banner_trusted_by"
  | "home_banner_verified_guarantee"
  | "home_banner_need_pro"
  | "home_banner_book_trusted"
  | "home_banner_shopping"
  | "home_banner_discover_goods"
  | "preferencesTitle"
  | "preferencesSubTitle"
  | "selectRoleHomeTitle"
  | "selectRoleHomeSubtitle"
  | "selectRoleMapTitle"
  | "selectRoleMapSubtitle"
  | "selectRoleBookingTitle"
  | "selectRoleBookingSubtitle"
  | "emailInvalid"
  | "emailRequired"
  | "passwordMin"
  | "passwordComplexity"
  | "passwordRequired"
  | "email_verification"
  | "email_verification_subtitle"
  | "email_verification_success"
  | "email_verification_success_description"
  | "signUp"
  | "signUpSubtitle"
  | "firstNameRequired"
  | "firstNameMin"
  | "lastNameRequired"
  | "lastNameMin"
  | "phoneNumberRequired"
  | "countryCodeRequired"
  | "countryRequired"
  | "birthDateRequired"
  | "genderRequired"
  | "genderRequired"
  | "genderRequired"
  | "genderRequired"
  | "by_signup_to_accept"
  | "privacy_policy"
  | "first_name"
  | "last_name"
  | "email_address"
  | "password"
  | "phone_number"
  | "whatsapp_number"
  | "same_as_whatsapp"
  | "country"
  | "birth_date"
  | "gender"
  | "select_gender"
  | "male"
  | "female"
  | "other"
  | "prefer_not_to_say"
  | "select"
  | "prefer"
  | "signup"
  | "skip"
  | "forgetPassword"
  | "forgetPasswordSubtitle"
  | "codeRequired"
  | "codeMin"
  | "resetPassword"
  | "resetPasswordSubtitle"
  | "passwordMatch"
  | "confirmPasswordRequired"
  | "newPassword"
  | "confirmPassword"
  | "code"
  | "preferences"
  | "category"
  | "continue"
  | "verify_documents"
  | "imageRequired"
  | "documentNumberRequired"
  | "documentNumberInvalid"
  | "documentNumberLength"
  | "aadhar_number"
  | "front_image"
  | "back_image"
  | "profile_pic"
  | "police_verification"
  | "upload_image"
  | "take_photo"
  | "retake"
  | "verify_now"
  | "camera_error"
  | "upload_error"
  | "front_image_required"
  | "back_image_required"
  | "profile_pic_required"
  | "schedule"
  | "scheduleSubtitle"
  | "alreadyHaveAnAccount"
  | "resend_otp"
  | "verify"
  | "privacyPolicytitle"
  | "termsConditionsTitle"
  | "lastUpdatedAt"
  | "something_went_wrong"
  | "home"
  | "dashboard"
  | "store"
  | "services"
  | "events"
  | "bookings"
  | "search"
  | "orders"
  | "chat"
  | "less"
  | "more"
  | "readmore"
  | "seeall"
  | "expand"
  | "startConversation"
  | "noComments"
  | "commentPlaceholder"
  | "sameTimeForAllDays"
  | "setYourSchedule"
  | "defineYourAvailability"
  | "available24By7"
  | "saveAndContinue"
  | "addPhoto"
  | "addVideo"
  | "addToStory"
  | "yourStory"
  | "deleteStory"
  | "deleteStoryConfirm"
  | "cancel"
  | "delete"
  | "share"
  | "addCaption"
  | "preview"
  | "invalidFileType"
  | "invalidFileType"
  | "myAccount"
  | "personalInfo"
  | "saved"
  | "blogs"
  | "home_latest_blogs_title"
  | "walletsRewards"
  | "paymentMethods"
  | "myCoupons"
  | "giftCard"
  | "logout"
  | "myPosts"
  | "myPackage"
  | "affiliateProducts"
  | "bankInfo"
  | "myDocuments"
  | "mySchedule"
  | "manageSocialAcc"
  | "settings"
  | "changePassword"
  | "addresses"
  | "notifications"
  | "blocked"
  | "faqs"
  | "helpSupport"
  | "rcPolicy"
  | "privacySecurity"
  | "termsConditions"
  | "noBioAvailable"
  | "continueReading"
  | "followers"
  | "following"
  | "personalInfoTitle"
  | "verified"
  | "birth_date"
  | "name"
  | "email"
  | "bio"
  | "deleteAccount"
  | "editPersonalInfo"
  | "bioPlaceholder"
  | "save"
  | "deleteAccountTitle"
  | "deleteAccountDescription"
  | "deleteAccountConfirm"
  | "deleteAccountCancel"
  | "logoutTitle"
  | "logoutDescription"
  | "logoutConfirm"
  | "logoutCancel"
  | "followBack"
  | "unfollow"
  | "follow"
  | "noFollowersYet"
  | "noFollowingYet"
  | "loadingFollowers"
  | "loadingFollowing"
  | "products"
  | "serviceProviders"
  | "suppliers"
  | "brands"
  | "noItemsSaved"
  | "nationalIdCopy"
  | "frontSide"
  | "backSide"
  | "customerService"
  | "writeUsAt"
  | "typeOfIssues"
  | "writeYourIssues"
  | "writeHere"
  | "submit"
  | "selectIssueType"
  | "orderRelated"
  | "paymentRelated"
  | "deliveryRelated"
  | "currentPasswordRequired"
  | "newPasswordRequired"
  | "oldPassword"
  | "enterNewPassword"
  | "enterConfirmPassword"
  | "confirmNewPassword"
  | "newPasswordNotMatch"
  | "passwordChangedSuccessfully"
  | "passwordChangedSuccessfullyDescription"
  | "notificationTitle"
  | "noNotifications"
  | "noNotificationsDescription"
  | "markAsRead"
  | "markAllAsRead"
  | "deleteNotification"
  | "newNotification"
  | "viewDetails"
  | "notificationServiceRequest"
  | "notificationBookingConfirmed"
  | "notificationBookingCancelled"
  | "notificationBookingCompleted"
  | "justNow"
  | "minutesAgo"
  | "hoursAgo"
  | "daysAgo"
  | "weeksAgo"
  | "monthsAgo"
  | "starting"
  | "ending"
  | "breakStarting"
  | "breakEnding"
  | "addBreak"
  | "removeBreak"
  | "update"
  | "monday"
  | "tuesday"
  | "wednesday"
  | "thursday"
  | "friday"
  | "saturday"
  | "sunday"
  | "scheduleUpdatedSuccessfully"
  | "ok"
  | "addAddress"
  | "editAddress"
  | "deleteAddress"
  | "setAsDefault"
  | "home"
  | "office"
  | "addressDeleted"
  | "addressDeletedDescription"
  | "addressAdded"
  | "addressAddedDescription"
  | "addressUpdated"
  | "addressUpdatedDescription"
  | "deleteAddressConfirm"
  | "addressName"
  | "buildingNo"
  | "floor"
  | "address"
  | "landmark"
  | "pincode"
  | "cityTown"
  | "state"
  | "addressNameRequired"
  | "addressRequired"
  | "pincodeRequired"
  | "cityTownRequired"
  | "stateRequired"
  | "countryRequired"
  | "noAddressesYet"
  | "addYourFirstAddress"
  | "default"
  | "deleteAddressDescription"
  | "addNewAddress"
  | "enterAddressName"
  | "saveAsDefault"
  | "add"
  | "enterCode"
  | "enterAccountNumber"
  | "buildingNoMax"
  | "floorMax"
  | "addressMax"
  | "pincodeInvalid"
  | "pincodeRequired"
  | "cityMax"
  | "cityRequired"
  | "stateMax"
  | "stateRequired"
  | "countryMax"
  | "countryRequired"
  | "landmarkMax"
  | "enterLandmark"
  | "selectCountry"
  | "enterPincode"
  | "selectState"
  | "selectCityTown"
  | "enterAddress"
  | "geolocationNotSupported"
  | "failedToGetLocation"
  | "locationPermissionDenied"
  | "locationInformationUnavailable"
  | "locationRequestTimedOut"
  | "myPosts"
  | "noPosts"
  | "noPostsDescription"
  | "loadingPosts"
  | "postsCount"
  | "closeViewer"
  | "viewComments"
  | "hideComments"
  | "previousPost"
  | "nextPost"
  | "addPost"
  | "createPost"
  | "selectImage"
  | "selectImages"
  | "selectMedia"
  | "dragDropImage"
  | "captionPlaceholder"
  | "selectVisibility"
  | "public"
  | "friends"
  | "private"
  | "postCreated"
  | "postCreatedDescription"
  | "postCreationFailed"
  | "imageRequired"
  | "captionMaxLength"
  | "maxImagesReached"
  | "invalidImageFormat"
  | "imageSizeTooLarge"
  | "uploadingPost"
  | "compressingVideo"
  | "removeImage"
  | "invalidMediaFormat"
  | "dragDropMedia"
  | "errorLoadingMap"
  | "loadingMap"
  | "failedToSelectLocation"
  | "noLocationsFound"
  | "googleMapsApiKeyNotConfigured"
  | "services_for_you"
  | "services_for_you_subtitle"
  | "all_categories"
  | "search_services_placeholder"
  | "categories_label"
  | "services_found"
  | "no_services_found"
  | "no_services_found_description"
  | "adjust_filters"
  | "try_different_search"
  | "sendMessage"
  | "posts"
  | "failedToLoadProfile"
  | "chatTooltip"
  | "locationTooltip"
  | "bookmarkTooltip"
  | "closeDrawer"
  | "failedToLoadPosts"
  | "failedToLoadServices"
  | "noPostsFound"
  | "noServicesFound"
  | "newServiceProvider"
  | "discoverServices"
  | "plumber"
  | "electrician"
  | "carpenter"
  | "painter"
  | "cleaner"
  | "gardener"
  | "findYourServices"
  | "findYourServicesDescription"
  | "ask_anything_placeholder"
  | "kartAi"
  | "kartAiGreeting"
  | "find"
  | "neatMyArea"
  | "supplier_registration_title"
  | "supplier_registration_subtitle"
  | "register_as_supplier"
  | "enter_code"
  | "enquiries"
  | "lead_details"
  | "addProduct"
  | "editProduct"
  | "productName"
  | "productPrice"
  | "productCategory"
  | "productDescription"
  | "productImages"
  | "productStock"
  | "actions"
  | "edit"
  | "delete"
  | "save"
  | "cancel"
  | "noProductsFound"
  | "noEnquiriesFound"
  | "message"
  | "status"
  | "date"
  | "customer"
  | "productService"
  | "business_type"
  | "id_proof_type"
  | "store_name"
  | "categories_served"
  | "operating_locations"
  | "contact_preferences"
  | "primary_mobile"
  | "show_phone"
  | "show_whatsapp"
  | "allow_calls"
  | "allow_chat"
  | "enquiry_only"
  | "loading"
  | "topProviders"
  | "topServices"
  | "noSuggestionsAvailable"
  | "general"
  | "cleaning"
  | "repairs"
  | "education"
  | "wellness"
  | "all"
  | "noBlogsFound"
  | "searchResults"
  | "searchResultsFor"
  | "noResultsFound"
  | "tryDifferentSearch"
  | "enterSearchQuery"
  | "users"
  | "seeAllResults"
  | "dashboard"
  | "dashboardOverview"
  | "salesGrowth"
  | "totalEarnings"
  | "totalActiveEvents"
  | "totalActiveServices"
  | "pendingOrders"
  | "newMessages"
  | "totalCollaborations"
  | "collaborationRequests"
  | "affiliateProducts"
  | "revenueCharts"
  | "revenueEarnedIn"
  | "monthly"
  | "weekly"
  | "yearly"
  | "upcomingBookings"
  | "upcomingEvents"
  | "recentTransactions"
  | "haircutAyurvedaSpa"
  | "fullPaid"
  | "start"
  | "exploringCulinaryDelights"
  | "booked"
  | "offline"
  | "ultralipNourishingShine"
  | "commissionRate"
  | "company"
  | "aboutUs"
  | "careers"
  | "press"
  | "blog"
  | "contactUs"
  | "contactUsDescription"
  | "firstName"
  | "lastName"
  | "phoneNumber"
  | "message"
  | "messageRequired"
  | "messageMin"
  | "sendMessage"
  | "contactUsSuccess"
  | "ourLocation"
  | "support"
  | "becomeServiceProvider"
  | "safetyCenter"
  | "communityGuidelines"
  | "accessibility"
  | "legal"
  | "cookiePolicy"
  | "refundPolicy"
  | "cancellationPolicy"
  | "allRightsReserved"
  | "careersSubtitle"
  | "joinOurTeam"
  | "careersHeroDescription"
  | "whyJoinUs"
  | "competitiveSalary"
  | "competitiveSalaryDesc"
  | "learningDevelopment"
  | "learningDevelopmentDesc"
  | "collaborativeCulture"
  | "collaborativeCultureDesc"
  | "careerGrowth"
  | "careerGrowthDesc"
  | "ourCulture"
  | "cultureDescription"
  | "openPositions"
  | "seniorSoftwareEngineer"
  | "productManager"
  | "marketingSpecialist"
  | "customerSuccessManager"
  | "engineering"
  | "product"
  | "marketing"
  | "customerSuccess"
  | "remote"
  | "hybrid"
  | "onSite"
  | "fullTime"
  | "applyNow"
  | "dontSeePosition"
  | "sendResume"
  | "sendYourResume"
  | "becomeServiceProviderSubtitle"
  | "joinAsServiceProvider"
  | "becomeServiceProviderHero"
  | "whyJoinAsProvider"
  | "growYourBusiness"
  | "growYourBusinessDesc"
  | "reachMoreCustomers"
  | "reachMoreCustomersDesc"
  | "securePayments"
  | "securePaymentsDesc"
  | "businessInsights"
  | "businessInsightsDesc"
  | "dedicatedSupport"
  | "dedicatedSupportDesc"
  | "verifiedProfile"
  | "verifiedProfileDesc"
  | "howItWorks"
  | "createAccount"
  | "createAccountDesc"
  | "completeProfile"
  | "completeProfileDesc"
  | "verifyDocuments"
  | "verifyDocumentsDesc"
  | "startEarning"
  | "startEarningDesc"
  | "requirements"
  | "validBusinessLicense"
  | "professionalExperience"
  | "qualityServiceDelivery"
  | "responsiveCustomerService"
  | "complianceWithStandards"
  | "joinThousands"
  | "providerSuccessStory"
  | "readyToStart"
  | "startYourJourney"
  | "getStartedNow"
  | "registerNow"
  | "whatAreCookies"
  | "cookiesDescription"
  | "howWeUseCookies"
  | "cookiesUsageDescription"
  | "essentialCookies"
  | "essentialCookiesDescription"
  | "analyticsCookies"
  | "analyticsCookiesDescription"
  | "marketingCookies"
  | "marketingCookiesDescription"
  | "preferenceCookies"
  | "preferenceCookiesDescription"
  | "managingCookies"
  | "managingCookiesDescription"
  | "thirdPartyCookies"
  | "thirdPartyCookiesDescription"
  | "cookieConsent"
  | "cookieConsentDescription"
  | "updatesToPolicy"
  | "cookiePolicyUpdatesDescription"
  | "cookiePolicyContactDescription"
  | "currentlyNoOpenings"
  | "whyJoinUsSubtitle"
  | "submitYourResume"
  | "submitResumeDescription"
  | "fullName"
  | "interestedPosition"
  | "yearsOfExperience"
  | "coverLetter"
  | "coverLetterPlaceholder"
  | "uploadResume"
  | "resumeFileTypes"
  | "submitApplication"
  | "submitting"
  | "thankYou"
  | "resumeSubmittedSuccess"
  | "storeComingSoon"
  | "storeComingSoonDescription"
  | "days"
  | "hours"
  | "minutes"
  | "seconds"
  | "launchingOn"
  | "whatToExpect"
  | "storeFeaturesSubtitle"
  | "wideSelection"
  | "wideSelectionDesc"
  | "fastDelivery"
  | "fastDeliveryDesc"
  | "securePayment"
  | "securePaymentDesc"
  | "qualityProducts"
  | "qualityProductsDesc"
  | "bestPrices"
  | "bestPricesDesc"
  | "multipleCategories"
  | "multipleCategoriesDesc"
  | "stayTuned"
  | "storeStayTunedDescription"
  | "reviews"
  | "see_all"
  | "no_reviews_yet"
  | "load_more_reviews"
  | "service_not_found"
  | "service_location"
  | "service_provider_location"
  | "loading"
  | "my_services"
  | "service_description"
  | "no_bookings_found"
  | "no_bookings_found_description"
  | "upcoming"
  | "completed"
  | "cancelled"
  | "totalServices"
  | "totalEvents"
  | "totalActiveServices"
  | "totalPendingBookings"
  | "totalCompletedBookings"
  | "totalBookings"
  | "bookingsCharts"
  | "latestReviews"
  | "page_not_found"
  | "page_not_found_description"
  | "go_back_home"
  | "go_back"
  | "error_404"
  | "pending"
  | "in_progress"
  | "accept"
  | "decline"
  | "mark_as_complete"
  | "otp_verification"
  | "enter_otp_description"
  | "verify_and_start"
  | "at_customer_location"
  | "at_provider_location"
  | "provider_service_at_customer_location"
  | "phoneNumberLength"
  | "phoneNumberInvalid"
  | "profileUpdated"
  | "profileUpdatedDescription"
  | "nameRequired"
  | "nameMin"
  | "contactRequired"
  | "contactInvalid"
  | "messageRequired"
  | "messageMin"
  | "countryCode"
  | "phoneMin"
  | "profileNotFound"
  | "copyLink"
  | "linkCopied"
  | "failedToCopy"
  | "copiedToClipboard"
  | "copyFailed"
  | "shareOnFacebook"
  | "shareOnTwitter"
  | "shareOnWhatsApp"
  | "shareViaEmail"
  | "noPostsFound"
  | "noServicesFound"
  | "phoneMin"
  | "auth_required_title"
  | "auth_required_description"
  | "getInTouch"
  | "viewProfile"
  | "shareProfile"
  | "noPostsFoundDescription"
  | "verified"
  | "active"
  | "inactive"
  | "waitingForApproval"
  | "copied"
  | "id"
  | "complete"
  | "confirm"
  | "cancel"
  | "start"
  | "cancel_booking_title"
  | "cancel_booking_description"
  | "yes_cancel"
  | "no_keep_booking"
  | "near"
  | "no_upcoming_bookings"
  | "no_upcoming_bookings_desc"
  | "goBack"
  | "no_reviews_yet_desc"
  | "businessListing"
  | "business"
  | "roleRequired"
  | "whatsappNumberRequired"
  | "whatsappNumberLength"
  | "whatsappNumberInvalid"
  | "whatsappCountryCodeRequired"
  | "businessInfo"
  | "businessInfoSubtitle"
  | "businessDetails"
  | "bankDetails"
  | "businessNameRequired"
  | "businessDescriptionRequired"
  | "businessAddressRequired"
  | "businessImagesRequired"
  | "businessImagesMin"
  | "businessImagesMax"
  | "maxImages"
  | "maxImagesReachedAllowed"
  | "businessName"
  | "businessDescription"
  | "businessAddress"
  | "businessImages"
  | "enterBusinessName"
  | "enterBusinessDescription"
  | "enterBusinessAddress"
  | "enterBusinessImages"
  | "description"
  | "editBusinessInfo"
  | "changeAddress"
  | "select_address"
  | "failed_to_load_services"
  | "failed_to_load_reviews"
  | "added_to_testimonials"
  | "removed_from_testimonials"
  | "failed_to_update_testimonial"
  | "testimonials"
  | "no_testimonials_selected"
  | "all_reviews"
  | "load_more"
  | "no_reviews_available"
  | "testimonial"
  // Leading Page
  | "listYourBusiness"
  | "startIn30Seconds"
  | "yourWhatsappNumber"
  | "yourNumber"
  | "getMyFreeListing"
  | "agreeToTerms"
  | "freeForever"
  | "growYourBusinessWith"
  | "freeListing"
  | "joinThousandsBusinessOwners"
  | "heroBenefit1"
  | "heroBenefit2"
  | "heroBenefit3"
  | "standOutHeadline"
  | "standOutHighlight"
  | "standOutRest"
  | "standOutSubtext"
  | "mobileNumber"
  | "startNow"
  | "chooseBusinessType"
  | "chooseBusinessTypeSubtext"
  | "serviceProviderDesc"
  | "serviceProviderExamples"
  | "supplierDesc"
  | "supplierExamples"
  | "featureFree"
  | "featureFreeDesc"
  | "featureEasy"
  | "guest_login"
  | "guest_login_description"
  | "continue_as_guest"
  | "featureEasyDesc"
  | "featurePersonalised"
  | "featurePersonalisedDesc"
  | "takeChargeTitle"
  | "takeChargeSubtext"
  | "addEssentialInfo"
  | "addEssentialInfoDesc"
  | "sharePhotosLogos"
  | "sharePhotosLogosDesc"
  | "showWhoYouAre"
  | "showWhoYouAreDesc"
  | "connectWithCustomersTitle"
  | "connectWithCustomersSubtext"
  | "showWhatYouOfferTitle"
  | "showWhatYouOfferSubtext"
  | "acceptOrdersTitle"
  | "acceptOrdersDesc"
  | "acceptOrdersLearnMore"
  | "beFoundForWhatYouSellTitle"
  | "beFoundForWhatYouSellDesc"
  | "beFoundLearnMore"
  | "offerYourServicesTitle"
  | "offerYourServicesDesc"
  | "offerServicesLearnMore"
  | "whatSuccessLooksLikeTitle"
  | "whatSuccessLooksLikeSubtext"
  | "showBestOfBusinessTitle"
  | "showBestOfBusinessSubtext"
  | "stepClaim"
  | "stepClaimDesc"
  | "stepPersonalise"
  | "stepPersonaliseDesc"
  | "stepManage"
  | "stepManageDesc"
  | "benefitWhyChooseUs"
  | "benefitHeading1"
  | "benefitHeading2"
  | "benefitSubheading"
  | "benefitServiceTitle"
  | "benefitServiceSub"
  | "benefitShopTitle"
  | "benefitShopSub"
  | "benefitSpl1Title"
  | "benefitSpl1Desc"
  | "benefitSpl2Title"
  | "benefitSpl2Desc"
  | "benefitSpl3Title"
  | "benefitSpl3Desc"
  | "benefitSup1Title"
  | "benefitSup1Desc"
  | "benefitSup2Title"
  | "benefitSup2Desc"
  | "benefitSup3Title"
  | "benefitSup3Desc"
  | "threeSimpleSteps"
  | "yourQuestionsAnswered"
  | "expandAll"
  | "collapseAll"
  | "statLabel1"
  | "statLabel2"
  | "statLabel3"
  | "successStoryTagline1"
  | "successStoryTagline2"
  | "successStoryTagline3"
  | "roleAstrologer"
  | "roleTileDealer"
  | "roleMakeupArtist"
  | "roleTailor"
  | "roleKiranaOwner"
  | "roleElectrician"
  | "step1Title"
  | "step1Desc"
  | "step2Title"
  | "step2Desc"
  | "step3Title"
  | "step3Desc"
  | "step4Title"
  | "step4Desc"
  | "freeListingBenefit1"
  | "freeListingBenefit2"
  | "freeListingBenefit3"
  | "boostBenefit1"
  | "boostBenefit2"
  | "boostBenefit3"
  | "faq1Question"
  | "faq1Answer"
  | "faq2Question"
  | "faq2Answer"
  | "faq3Question"
  | "faq3Answer"
  | "faq4Question"
  | "faq4Answer"
  | "faq5Question"
  | "faq5Answer"
  | "faq6Question"
  | "faq6Answer"
  | "faq7Question"
  | "faq7Answer"
  | "faq8Question"
  | "faq8Answer"
  | "successStories"
  | "hearFromOwners"
  | "seeHowOthers"
  | "seeAllStories"
  | "howItWorks"
  | "getFreeListingSteps"
  | "takesLessThanMinutes"
  | "step"
  | "done"
  | "forBusinessOwners"
  | "connectWithNewCustomers"
  | "yourFreeListingPage"
  | "oneProfileMoreVisibility"
  | "completeBusinessProfile"
  | "wantMoreLeads"
  | "getMoreVisibility"
  | "promoteListingDesc"
  | "getStartedFree"
  | "commonQuestions"
  | "gotAQuestion"
  | "quickAnswers"
  | "by"
  | "quickAnswers"
  | "booking_completed"
  | "how_was_your_experience"
  | "for"
  | "maybe_later"
  | "rate_now"
  | "write_review"
  | "submit_review"
  | "no_detailed_ratings_available"
  | "collapse_review"
  | "expand_review"
  | "yes"
  | "no"
  | "getQuote"
  | "priceCatalog"
  | "price_catalog_subtitle"
  | "price_catalog_list_subtitle"
  | "tap_to_view_full"
  | "other_services"
  | "time_slots_available"
  | "instant_booking"
  | "review_submitted_success"
  | "review_submit_failed"
  | "review_answer_placeholder"
  | "ai_thinking"
  | "ai_services_found"
  | "ai_error_message"
  | "ai_try_asking"
  | "ai_services_label"
  | "ai_explore_categories"
  | "ai_voice_tap"
  | "ai_voice_listening"
  | "ai_voice_not_supported"
  | "messageIsTooShort"
  | "messageSentSuccessfully"
  | "messageSentDescription"
  | "close"
  | "myReviews"
  | "latest_leads"
  | "no_leads_yet"
  | "no_leads_desc"
  | "totalPhoneNumberViews"
  | "totalProfileViews"
  | "advertisement"
  // Advertise Page
  | "create_ad"
  | "error_loading_advertisements"
  | "error_loading_ads_description"
  | "no_advertisements_yet"
  | "no_advertisements_description"
  | "create_your_first_ad"
  | "delete_advertisement"
  | "delete_ad_confirmation"
  | "this_advertisement"
  | "edit_advertisement"
  | "create_advertisement"
  // Ad Filters
  | "filters"
  | "clear_filters"
  | "all_services"
  | "ad_status_all"
  | "ad_status_pending"
  | "ad_status_active"
  | "ad_status_inactive"
  | "ad_status_rejected"
  // Ad Card
  | "untitled_advertisement"
  | "no_description_available"
  | "ad_views"
  | "ad_clicks"
  | "ad_rejection_reason"
  // Ad Form
  | "ad_form_service"
  | "ad_form_select_service"
  | "ad_form_title"
  | "ad_form_description"
  | "ad_form_image"
  | "ad_form_start_date"
  | "ad_form_end_date"
  | "ad_form_click_to_upload"
  | "ad_form_rejection_reason_label"
  | "ad_form_waiting_approval"
  | "ad_form_under_review"
  | "ad_form_advertisement_status"
  | "update_advertisement"
  // Ad Form Validation
  | "ad_validation_service_required"
  | "ad_validation_image_required"
  | "ad_validation_start_date_required"
  | "ad_validation_end_date_required"
  | "ad_validation_end_after_start"
  // My Store
  | "my_store"
  | "addNewProduct"
  | "basic_info"
  | "product_name"
  | "enter_product_name"
  | "sku_no"
  | "enter_sku_no"
  | "select_category"
  | "select_subcategory"
  | "select_type"
  | "select_brand"
  | "other_specifications"
  | "write_here"
  | "price"
  | "enter_price"
  // Product Form Validation Keys
  | "productCategoryRequired"
  | "productSubCategoryRequired"
  | "productNameRequired"
  | "productSkuRequired"
  | "productPriceRequired"
  | "productCurrencyRequired"
  | "productDescriptionRequired"
  | "productImagesRequired"
  | "productReturnableRequired"
  // Product Form Label Keys
  | "currency"
  | "is_returnable"
  | "select_currency"
  | "returnable_product"
  | "product_is_returnable"
  | "enter_value"
  | "productOriginRequired"
  | "productDescriptionMin"
  | "productImagesMin"
  | "select_origin"
  | "product_origin"
  | "product_created_successfully"
  | "product_updated_successfully"
  | "add_new_product"
  | "edit_product"
  | "update_product"
  | "product_updated"
  | "product_created"
  | "delete_product"
  | "delete_product_description"
  | "deleting"
  | "loadingMore"
  | "skuId"
  | "category"
  | "subCategory"
  | "stockAvailability"
  | "brand"
  | "status"
  | "action"
  | "product_deleted"
  | "product_deleted_description"
  | "go_to_my_store"
  | "go_back"
  | "failed_to_load_product_details"
  | "availability"
  | "rejection_reason"
  | "specifications"
  | "in_stock"
  | "out_of_stock"
  | "noProductsFoundDescription"
  | "view_details"
  | "enquiry_details"
  | "customer_name"
  | "quantity"
  | "time"
  | "respond"
  | "viewed"
  | "my_orders"
  | "search_orders"
  | "pending"
  | "completed"
  | "no_orders_found"
  | "storeLocation"
  | "yearEstablishedLabel"
  | "responseRateLabel"
  | "locationLabel"
  | "newArrivals"
  | "similarProductsDescription"
  | "directions"
  | "similar"
  | "uploadDocument"
  | "my_address"
  | "visiting_charge_inr"
  | "service_radius_km"
  | "provider_service_at_customer_location"
  | "this_service_have_a_specific_duration"
  | "firstNameMax"
  | "lastNameMax"
  | "kyc_id_proof_type_placeholder"
  | "kyc_pan_card_image"
  | "kyc_doc_hint_pan"
  | "kyc_id_proof_image"
  | "kyc_doc_hint_id"
  | "kyc_address_proof_image"
  | "kyc_doc_hint_address"
  | "kyc_complete_registration"
  | "kyc_pan_required"
  | "kyc_pan_invalid"
  | "kyc_pan_image_required"
  | "kyc_id_type_required"
  | "kyc_id_image_required"
  | "kyc_address_proof_required"
  | "kyc_owner_name_required"
  | "kyc_country_code_required"
  | "kyc_owner_mobile_required"
  | "kyc_owner_mobile_digits"
  | "kyc_owner_email_invalid"
  | "kyc_owner_email_required"
  | "kyc_section_gst"
  | "kyc_optional"
  | "kyc_gst_number"
  | "kyc_gst_state"
  | "kyc_gst_certificate"
  | "kyc_gst_number_placeholder"
  | "kyc_gst_state_placeholder"
  | "kyc_gst_certificate_placeholder"
  | "kyc_gst_number_invalid"
  | "kyc_gst_state_invalid"
  | "kyc_pan_number"
  | "kyc_section_owner"
  | "kyc_pan_number_placeholder"
  | "kyc_owner_name"
  | "kyc_owner_name_placeholder"
  | "kyc_owner_mobile"
  | "kyc_owner_mobile_placeholder"
  | "kyc_section_identity"
  | "kyc_id_proof_type"
  | "kyc_owner_email"
  | "kyc_owner_email_placeholder"
  | "store_setup_store_name_required"
  | "store_setup_invalid_email"
  | "store_setup_contact_phone_required"
  | "store_setup_contact_phone_digits"
  | "store_setup_categories_min"
  | "store_setup_categories_required"
  | "store_setup_basic_info"
  | "store_setup_display_name"
  | "store_setup_business_type"
  | "store_setup_display_name_placeholder"
  | "store_setup_business_type_placeholder"
  | "store_setup_website_url"
  | "store_setup_website_url_placeholder"
  | "store_setup_about_us"
  | "store_setup_about_us_placeholder"
  | "wholesaler"
  | "retailer"
  | "exporter"
  | "manufacturer"
  | "store_setup_categories_served"
  | "store_setup_operating_locations"
  | "store_setup_locations_placeholder"
  | "store_setup_categories_placeholder"
  | "store_setup_contact_preferences"
  | "store_setup_business_type"
  | "store_setup_aesthetics_branding"
  | "store_setup_store_logo"
  | "store_setup_doc_hint_logo"
  | "store_setup_store_banner"
  | "store_setup_doc_hint_banner"
  | "store_setup_store_location"
  | "store_setup_loading_addresses"
  | "store_setup_selected"
  | "store_setup_add_new_address"
  | "store_setup_contact_info"
  | "store_setup_primary_contact"
  | "store_setup_contact_phone_placeholder"
  | "store_setup_contact_email"
  | "store_setup_contact_email_placeholder"
  | "store_setup_establishment_year"
  | "store_setup_establishment_year_placeholder"
  | "store_setup_save_continue"
  | "postReel"
  | "reels"
  | "storeInfo"
  | "storeName"
  | "contactEmail"
  | "primaryMobile"
  | "categoriesServed"
  | "operatingLocations"
  | "businessType"
  | "establishmentYear"
  | "storeImages"
  | "storeInfoUpdated"
  | "storeInfoUpdatedDesc"
  | "editStoreInfo"
  | "storeLogo"
  | "storeBanner"
  | "address"
  | "no_address_yet"
  | "add_address"
  | "whatsappNumber"
  | "whatsappNumberRequired"
  | "deleteAddress"
  | "deleteAddressDescription"
  | "store_setup_address_required"
  | "store_setup_logo_required"
  | "store_setup_banner_required"
  | "store_setup_operating_locations_required"
  | "store_setup_establishment_year_required"
  | "websiteUrl"
  | "selectCurrentAddress"
  // Aibot
  | "aibot_greeting"
  | "aibot_tooltip"
  | "aibot_header_title"
  | "aibot_online_now"
  | "aibot_thinking"
  | "aibot_input_placeholder"
  | "aibot_powered_by"
  | "aibot_benefit_response"
  | "aibot_cost_response"
  | "aibot_feature_response"
  | "aibot_reach_response"
  | "aibot_default_response"
  | "showNumberInServices"
  | "submit_request"
  | "request_sent_successfully"
  | "quote_request_description"
  | "marketing_tools"
  | "pendingEnquiries"
  | "Enter_value"
  | "Select_option"
  | "please_provide_rating"
  | "rate_your_experience"
  ;

// Sample dictionaries
const dictionaries: Record<"en", Record<TranslationKey, string>> = {
  en: english,
  // es: {
  //   welcome_back: "¡Bienvenido de nuevo!",
  //   email_address: "Correo electrónico",
  //   password: "Contraseña",
  //   forgot_password: "¿Olvidaste tu contraseña?",
  //   login: "Iniciar sesión",
  //   continue_with_google: "Continuar con Google",
  //   no_account: "¿No tienes una cuenta?",
  //   sign_up: "Regístrate",
  //   brand_tagline: "Tu socio de 8 manos",
  //   continue_as: "Continuar como",
  //   customer: "Cliente",
  //   service_provider: "Proveedor de servicios",
  //   supplier: "Proveedor",
  //   login_subtitle:
  //     "Ingrese su correo electrónico y contraseña para iniciar sesión.",
  //   emailInvalid: "Correo electrónico no válido",
  //   emailRequired: "El correo electrónico es obligatorio",
  //   passwordMin: "La contraseña debe tener al menos 8 caracteres",
  //   passwordComplexity:
  //     "La contraseña debe contener mayúsculas, minúsculas, números y caracteres especiales",
  //   passwordRequired: "Se requiere contraseña",
  //   email_verification: "Verificar tu dirección de correo electrónico",
  //   email_verification_subtitle: "Por favor, ingresa el código de verificación enviado a tu correo electrónico registrado para verificar tu identidad.",
  // },
  // hi: {
  //   welcome_back: "वापसी पर स्वागत है!",
  //   email_address: "ईमेल पता",
  //   password: "पासवर्ड",
  //   forgot_password: "पासवर्ड भूल गए?",
  //   login: "लॉग इन करें",
  //   continue_with_google: "Google के साथ जारी रखें",
  //   no_account: "क्या आपके पास खाता नहीं है?",
  //   sign_up: "साइन अप करें",
  //   brand_tagline: "आपका 8 हाथों वाला साथी",
  //   continue_as: "के रूप में जारी रखें",
  //   customer: "ग्राहक",
  //   service_provider: "सेवा प्रदाता",
  //   supplier: "आपूर्तिकर्ता",
  //   login_subtitle: "लॉगिन करने के लिए अपना ईमेल और पासवर्ड दर्ज करें।",
  //   emailInvalid: "अमान्य ईमेल",
  //   emailRequired: "ईमेल आवश्यक है",
  //   passwordMin: "पासवर्ड कम से कम 8 वर्ण का होना चाहिए",
  //   passwordComplexity:
  //     "पासवर्ड में अपरकेस, लोअरकेस, नंबर और विशेष वर्ण होने चाहिए",
  //   passwordRequired: "पासवर्ड आवश्यक है",
  //   email_verification: "अपने ईमेल पते की पुष्टि करें",
  //   email_verification_subtitle: "कृपया अपनी पहचान सत्यापित करने के लिए अपने पंजीकृत ईमेल पते पर भेजा गया वन-टाइम पासवर्ड (OTP) दर्ज करें।",
  // },
};

interface TranslationContextType {
  locale: "en";
  setLocale: (locale: "en") => void;
  t: (key: TranslationKey, params?: Record<string, string | number>) => string;
}

const TranslationContext = createContext<TranslationContextType | undefined>(
  undefined,
);

export function TranslationProvider({ children }: { children: ReactNode }) {
  const [locale, setLocale] = useState<"en">("en");

  const t = (
    key: TranslationKey,
    params?: Record<string, string | number>,
  ): string => {
    let text = dictionaries[locale][key] || key;
    if (params) {
      Object.entries(params).forEach(([paramKey, value]) => {
        text = text.replace(`{${paramKey}}`, String(value));
      });
    }
    return text;
  };

  return (
    <TranslationContext.Provider value={{ locale, setLocale, t }}>
      {children}
    </TranslationContext.Provider>
  );
}

export function useTranslationContext() {
  const context = useContext(TranslationContext);
  if (context === undefined) {
    throw new Error(
      "useTranslationContext must be used within a TranslationProvider",
    );
  }
  return context;
}
